...
/// Local-authoritative programme state.
///
/// A phone that has already loaded/generated a programme must never wait for a
/// network round-trip to open Home/Today or start/finish a workout. Supabase is
/// a synchronization target; SharedPreferences is the immediate device state.
class ProgrammeStore {
  ...
  Future<StoredProgramme?> _cloudCurrent(String userId) async {
  ...
  void _syncLater(StoredProgramme value) {
    unawaited(_writeCloudOrQueue(_rowForStored(value)));
  }
  ...
  Future<StoredProgramme> ensureForProfile(Map<String, dynamic> profile) async {
    ...
    if (cached != null &&
        ...
        cached.hasSessions) {
      await ProgrammeAdaptationStore.ensureWeek(cached.currentWeek);
      return cached;
    }

    if (user != null && cached == null) {
      try {
        final cloud = await _cloudCurrent(user.id);
        if (cloud != null &&
            cloud.profileSignature == signature &&
            cloud.hasSessions) {
          await _cache(cloud);
          await ProgrammeAdaptationStore.ensureWeek(cloud.currentWeek);
          return cloud;
        }
      } catch (error) {
    ...
    if (user != null) _syncLater(created);
  ...
  Future<StoredProgramme?> loadCurrent() async {
    ...
    try {
      final cloud = await _cloudCurrent(user.id);
      if (cloud != null) await _cache(cloud);
      return cloud;
    } catch (error) {
      await AppErrorStore.record('Programme load', error);
      return null;
  ...
  Future<void> _advanceSession({
    ...
    if (profile != null) {
      try {
        final result = await AdaptiveProgrammeService(client)
            .buildNextWeek(
              profile: profile,
